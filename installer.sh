...
then
	echo "THE COMPUTER DOESN'T WORK A1Q OS INSTALLER. PLEASE RUN A1Q OS INSTALLER ON THE COMPUTER"
	exit
else
	if [ $(</etc/a1q-release) != "alpha-x-installer"  ];
	then
		echo "THE SYSTEM IS NOT RUNNING A1Q OS INSTALLER. PLEASE RUN THE INSTALLER ON A1Q OS INSTALLER"
		exit
	fi
fi
